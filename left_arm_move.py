...
def move1():
    ...
    try:
        joint_states = rospy.wait_for_message("left_arm/joint_states", JointState)
...
def left_arm_control_callback(target_pose):
    global client
    g = FollowJointTrajectoryGoal()
    g.trajectory = JointTrajectory()
    g.trajectory.joint_names = JOINT_NAMES
    try:
        ...
        g.trajectory.points = [JointTrajectoryPoint(positions=joints_pos, velocities=[0]*6, time_from_start=rospy.Duration(0.0))]
        g.trajectory.points.append(
                JointTrajectoryPoint(positions=target_pose.position, velocities=[0]*6, time_from_start=rospy.Duration(d)))
        client.send_goal(g)
        print("client_send_goal...")
        client.wait_for_result()
...
def main():
    global client
    try:
        rospy.init_node("left_arm_move", anonymous=True, disable_signals=True)   
        ...
        # rospy.Subscriber("left_arm/control_cmd", JointState, left_arm_control_callback)   
        if (index > 0):
