...
#include "left_ur_driver/ur_driver.h"
#include "left_ur_driver/ur_hardware_interface.h"
#include "left_ur_driver/do_output.h"
#include <string.h>
#include <vector>
#include <mutex>
...
public:
	RosWrapper(std::string host, int reverse_port) :
		...
		joint_names.push_back(joint_prefix + "l_shoulder_pan_joint");
		joint_names.push_back(joint_prefix + "l_shoulder_lift_joint");
		joint_names.push_back(joint_prefix + "l_elbow_joint");
		joint_names.push_back(joint_prefix + "l_wrist_1_joint");
		joint_names.push_back(joint_prefix + "l_wrist_2_joint");
		joint_names.push_back(joint_prefix + "l_wrist_3_joint");
		...
		if (robot_.start()) {
			...
			speed_sub_ = nh_.subscribe("left_arm/ur_driver/joint_speed", 1,
					&RosWrapper::speedInterface, this);
			urscript_sub_ = nh_.subscribe("left_arm/ur_driver/URScript", 1,
					&RosWrapper::urscriptInterface, this);

			io_srv_ = nh_.advertiseService("left_arm/ur_driver/set_io",
					&RosWrapper::setIO, this);
			payload_srv_ = nh_.advertiseService("left_arm/ur_driver/set_payload",
...
private:
		...
		if (!validateJointNames()) {
			...
			result_.error_string =
					"Received a goal with incorrect joint names: "
							+ outp_joint_names;
			gh.setRejected(result_, result_.error_string);
			printf("*********%ld",outp_joint_names.find("r_shoulder_pan_joint"));
			if (outp_joint_names.find("r_shoulder_pan_joint") != -1)
			{
				print_info("it is right arm goal! cancel....");
				// return;
			}
			print_error(result_.error_string);
			return;
	...
	void rosControlLoop() {
		...
		realtime_tools::RealtimePublisher<geometry_msgs::TwistStamped> tool_vel_pub( nh_, "left_arm/tool_velocity", 1 );
		tool_vel_pub.msg_.header.frame_id = base_frame_;

		// realtime_tools::RealtimePublisher<std_msgs::String> tool_tcp_pub( nh_, "left_arm/tool_tcp_point", 1 );
	...
	void publishRTMsg() {
		ros::Publisher joint_pub = nh_.advertise<sensor_msgs::JointState>(
				"left_arm/joint_states", 1);
		ros::Publisher common_joint_pub = nh_.advertise<sensor_msgs::JointState>(
				"joint_states", 1);
		ros::Publisher wrench_pub = nh_.advertise<geometry_msgs::WrenchStamped>(
				"left_arm/wrench", 1);
        ros::Publisher tool_vel_pub = nh_.advertise<geometry_msgs::TwistStamped>("left_arm/tool_velocity", 1);
        static tf::TransformBroadcaster br;

		ros::Publisher actual_tcp_pose_pub = nh_.advertise<geometry_msgs::TwistStamped>("left_arm/actual_tcp_pose", 1);
        
		ros::Publisher yc_pub = nh_.advertise<public_pkg::status_analog_msg>(
				"ruijia/RJ1300/yc", 1);
		ros::Publisher yx_pub = nh_.advertise<public_pkg::status_digital_msg>(
				"ruijia/RJ130/yx", 1);
		
		public_pkg::status_analog_msg msg_yc;
		msg_yc.header.srcNodeName = "armsInfoAnalog";
		msg_yc.header.srcNodeType = "arm";
		msg_yc.header.msgType = "public_pkg/status_analog_msg";

		public_pkg::status_digital_msg msg_yx;
		msg_yx.header.srcNodeName = "armsInfoDigital";
		msg_yx.header.srcNodeType = "arm";
		msg_yx.header.msgType = "public_pkg/status_digital_msg";
		...
		while (ros::ok()) {
			...
			// msg_yc.header.msgTime = ros::Time::now();
			// const std::string content = "{\"analogArray\": [{\"seq\": 1, \"type\": \"float\", \"value\": 1, \"name\": \"l_shoulder_pan_joint\"}]}";
			// msg_yc.content = content;
			// yc_pub.publish(msg_yc);
			common_joint_pub.publish(joint_msg);
			...
			// publish tcp_position
			// printf("tool_point:[x,y,z,rx,ry,rz]:[%f,%f,%f,%f,%f,%f]\n",tool_vector_actual[0],tool_vector_actual[1],tool_vector_actual[2],tool_vector_actual[3],tool_vector_actual[4],tool_vector_actual[5]);
			geometry_msgs::TwistStamped ac_tcp_pose;
			ac_tcp_pose.header.frame_id = base_frame_;
            ac_tcp_pose.header.stamp = joint_msg.header.stamp;
			ac_tcp_pose.twist.linear.x = tool_vector_actual[0];
			ac_tcp_pose.twist.linear.y = tool_vector_actual[1];
			ac_tcp_pose.twist.linear.z = tool_vector_actual[2];
			ac_tcp_pose.twist.angular.x = tool_vector_actual[3];
			ac_tcp_pose.twist.angular.y = tool_vector_actual[4];
			ac_tcp_pose.twist.angular.z = tool_vector_actual[5];
			actual_tcp_pose_pub.publish(ac_tcp_pose);
			...
			if (robot_.sec_interface_->robot_state_->isRobotConnected()){
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 1, \"type\": \"int\", \"value\": 1, \"name\": \"l_arm_connected\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}else{
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 1, \"type\": \"int\", \"value\": 0, \"name\": \"l_arm_connected\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}
			// std::cout << (robot_.sec_interface_->robot_state_->isRobotConnected()) << std::endl;
			if (robot_.sec_interface_->robot_state_->isPowerOnRobot()) {
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 3, \"type\": \"int\", \"value\": 1, \"name\": \"l_arm_power_on\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}else{
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 3, \"type\": \"int\", \"value\": 0, \"name\": \"l_arm_power_on\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}
			if (robot_.sec_interface_->robot_state_->isRealRobotEnabled()) {
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 2, \"type\": \"int\", \"value\": 1, \"name\": \"l_arm_enabled\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}else{
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 2, \"type\": \"int\", \"value\": 0, \"name\": \"l_arm_enabled\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}
			...
			if (robot_.sec_interface_->robot_state_->isEmergencyStopped()) {
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 4, \"type\": \"int\", \"value\": 1, \"name\": \"l_arm_emergency_stopped\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}else{
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 4, \"type\": \"int\", \"value\": 0, \"name\": \"l_arm_emergency_stopped\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}
			...
			if (robot_.sec_interface_->robot_state_->isProtectiveStopped()) {
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 5, \"type\": \"int\", \"value\": 1, \"name\": \"l_arm_protective_stopped\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}else{
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 5, \"type\": \"int\", \"value\": 0, \"name\": \"l_arm_protective_stopped\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}
			...
			if (robot_.sec_interface_->robot_state_->isProgramRunning()) {
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 6, \"type\": \"int\", \"value\": 1, \"name\": \"l_arm_program_running\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}else{
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 6, \"type\": \"int\", \"value\": 0, \"name\": \"l_arm_program_running\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}
			
			if (robot_.sec_interface_->robot_state_->isProgramPaused()) {
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 7, \"type\": \"int\", \"value\": 1, \"name\": \"l_arm_program_paused\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}else{
				msg_yx.header.msgTime = ros::Time::now();
				const std::string content = "{\"digitalArray\": [{\"seq\": 7, \"type\": \"int\", \"value\": 0, \"name\": \"l_arm_program_paused\"}]}";
				msg_yx.content = content;
				yx_pub.publish(msg_yx);
			}
	...
	void publishMbMsg() {
		...
		ros::Publisher io_pub = nh_.advertise<ur_msgs::IOStates>(
				"left_arm/ur_driver/io_states", 1);
